...
class PackageDates (models.Model):
    ...
    def clean(self):
        if self.ift_date is not None and self.opening_date is not None and self.ift_date >= self.opening_date:
            raise ValidationError({'opening_date':
            _('Tender Opening Date should be greater than tender publish date')})
        if self.opening_date is not None and self.evaluation_submission_date is not None and self.opening_date > self.evaluation_submission_date:
            raise ValidationError({'evaluation_submission_date':
            _('Evaluation Submission Date should be greater than tender opening date')})
        if self.evaluation_submission_date is not None and self.evaluation_approval_date is not None and self.evaluation_submission_date > self.evaluation_approval_date:
        ...
        if self.noa_date is not None and self.contract_sign_date is not None and self.noa_date > self.contract_sign_date:
            raise ValidationError({'contract_sign_date':
            _('Contract Signing Date should be equal or greater than NOA date')})
        if self.contract_sign_date is not None and self.start_date is not None and self.contract_sign_date > self.start_date:
...
class FileUp(models.Model):
    package_no = models.ForeignKey(Packages, related_name='rnfile', on_delete=models.CASCADE)
